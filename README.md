...
d(f_B)/dt = [D_W·f_W + D_D·f_D - (D_W + D_F + D_M + D_G)·f_B] / (Z_B·V_B)

Where:
- `f_B`: Fugacity in fish body (Pa)
- `t`: Time (hours)
- `D_W`: Gill uptake D-value (mol/Pa·h)
- `f_W`: Fugacity in water (Pa)
- `D_D`: Dietary uptake D-value (mol/Pa·h)
- `f_D`: Fugacity in food (Pa)
- `D_F`: Fecal excretion D-value (mol/Pa·h)
- `D_M`: Metabolic transformation D-value (mol/Pa·h)
- `D_G`: Growth dilution D-value (mol/Pa·h)
- `Z_B`: Fugacity capacity of fish (mol/Pa·m³)
- `V_B`: Fish body volume (m³)
...
Z_N = 0.035 × K_OW / H # Non-lipid organic matter fugacity capacity
Z_B = F_L·Z_O + F_N·Z_N + F_W·Z_W # Fish body fugacity capacity
Z_D = F_LD·Z_O + F_ND·Z_N + F_WD·Z_W # Food fugacity capacity
...
k1 = Δt × dfdt(t_n, f_n)
k2 = Δt × dfdt(t_n + Δt/2, f_n + k1/2)
k3 = Δt × dfdt(t_n + Δt/2, f_n + k2/2)
k4 = Δt × dfdt(t_n + Δt, f_n + k3)
...
|----------|-----------|---------|---------------|------|-------------|
| **Fipronil Properties** | Henry's Law Constant | H | 2.4 × 10⁻³ | Pa·m³/mol | Air-water partitioning coefficient |
| | Octanol-Water Partition Coefficient (log) | logKow | 4.0 | dimensionless | Hydrophobicity indicator |
| | Degradation Half-Life | t₁/₂ | 72.0 | hours | Metabolic transformation rate |
...
| | Gill Ventilation Rate | G_W | 0.02 | m³/h | Water flow across gills |
| | Gill Absorption Efficiency | E_W | 0.75 | dimensionless | Extraction efficiency at gills |
| | Feeding Rate | G_D | 0.005 | m³/h | Volume of food processed per hour |
| | Intestinal Absorption Efficiency | E_D | 0.85 | dimensionless | Digestive tract absorption |
| | Food Absorption Fraction | β | 0.85 | dimensionless | Fraction absorbed from food |
| | Intestine-Fish Distribution Coefficient | K_DG | 2.0 | dimensionless | Gut-to-body transfer ratio |
| | Growth Rate Constant | k_g | 0.001 | h⁻¹ | Specific growth rate |
| **Environmental** | Water Fugacity | f_W | 1.2 × 10⁻⁵ | Pa | Thermodynamic activity in water |
| | Food Fugacity | f_D | 3.5 × 10⁻⁵ | Pa | Thermodynamic activity in food |
...
The prediction band (90% confidence interval) is generated by perturbing four sensitive parameters by ±5%:
...
C_upper = max[C_base, C(H×1.05), C(logKow×1.05), C(f_W×1.05), C(G_W×1.05)]
C_lower = min[C_base, C(H×0.95), C(logKow×0.95), C(f_W×0.95), C(G_W×0.95)]

## 🔧 Unit Conversions
Residue concentrations are converted from fugacity to practical units:
C (ng/g) = f_B × Z_B × MW × 1000
Where:
- `f_B`: Fish fugacity (Pa)
- `Z_B`: Fish fugacity capacity (mol/Pa·m³)
